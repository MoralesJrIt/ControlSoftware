...
public class ingresar_devolucion extends javax.swing.JInternalFrame {
    ...
    public ingresar_devolucion() {
        ...
        if (bd.valor("select max(cast(d.n_devolucion as unsigned))+1 as maximo from devoluciones d inner join factura f on d.id_factura=f.idfactura where d.cia=" + ControlSoftware.cia + " and f.tipo=1") == null) {
            nc_fiscal.setText("1");
        } else {
            nc_fiscal.setText(String.format("%.0f", (Double.parseDouble(bd.valor("select max(cast(d.n_devolucion as unsigned))+1 as maximo from devoluciones d inner join factura f on d.id_factura=f.idfactura where d.cia=" + ControlSoftware.cia + " and f.tipo=1")))));
        }
