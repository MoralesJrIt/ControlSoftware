...
//        jButton5.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/back.png")));
        ...
        if (factura.getText().isEmpty()) {
            ...
            if (contado.isSelected()) {
                ...
                if (opc == 0) {
                    ...
                    String cf = bd.doble("INSERT INTO `factura`(`numerofactura`, `tipo`, `cia`, `idcliente`, `idvendedor`, `subtotal`, `isv`, `total`, `fechafactura`, `fechaingreso`, `saldo`, `idusuario`, `observacion`, `condicionpago`, `transporte`, `guia`, `estado`, `comprobado`, `op`, `caiid`, `cajaid`) VALUES ('" + factura.getText() + "','" + l + "','" + ControlSoftware.cia + "','" + codcliente.getText() + "','" + ve.getId() + "','" + st + "','" + im + "','" + t + "','" + Date_Format.format(fecha.getDate()) + "','" + date.format(formatter) + "','" + sal + "','" + ControlSoftware.us + "','" + obsfact.getText() + "','" + m + "','0','pendiente','1', '0','" + pedido + "', '" + caf + "', '" + bd.valor("select id from caja where idusuario="+ControlSoftware.us+" and cia="+ControlSoftware.cia+" and estado=1") + "')", "SELECT @@IDENTITY AS 'id'");
    ...
    private void codclienteFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_codclienteFocusLost
        ...
        if (bd.valor("select nombre from clientes where idcliente=" + codcliente.getText()+" and cia="+ControlSoftware.cia).equals("")) {
            ...
            descliente.setText("" + bd.valor("select nombre from clientes where idcliente=" + codcliente.getText()+" and cia="+ControlSoftware.cia).equals(""));
        }
