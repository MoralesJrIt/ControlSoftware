...
public class recibo extends javax.swing.JInternalFrame {
    ...
    public recibo(String cl, String f) {
        ...
        if (bd.valor("select MAX(cast(numero_recibo as unsigned))+1 from recibopago where tipo=0 and cia=" + ControlSoftware.cia) == null) {
            nrecibo.setText("1");
        } else {
            nrecibo.setText(String.format("%.0f", (Double.parseDouble(bd.valor("select MAX(cast(numero_recibo as unsigned))+1 from recibopago where tipo=0 and cia=" + ControlSoftware.cia)))));
        }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        datosrecibo l = new datosrecibo(Double.parseDouble(gtotal.getText()));
    ...
    private void clieActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_clieActionPerformed
        // TODO add your handling code here:
        seleccionarcliente_recibo l = new seleccionarcliente_recibo();
        Dimension internalFrameSize = l.getSize();
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        l.setLocation((screenSize.width - internalFrameSize.width) / 2,
                (screenSize.height - internalFrameSize.height) / 2);
